    ...
    static class Program
    {
        ...
        static async Task Main()
        {
            try
            {
                ...
                {
#if MACOS
                    MacOSStartup.Initialize();
#elif LINUX
                    LinuxStartup.Initialize();
#endif
                });
                
                if (await Task.WhenAny(startupTask, Task.Delay(5000)) != startupTask)
                    throw new TimeoutException("Startup timed out");
        ...
        public static void LoadBSXRepository(string xmlPath)
        {
            ...
            {
                //not proper XML
                MessageBox.Show("This is not a Satellaview server XML file.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
